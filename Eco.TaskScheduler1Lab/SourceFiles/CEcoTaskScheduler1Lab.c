...
#define MAX_STATIC_TASK_COUNT 8
 ...
 *
 */
int16_t ECOCALLMETHOD CEcoTaskScheduler1Lab_C761620F_NewTask(/*in*/ IEcoTaskScheduler1Ptr_t me, /*in*/ voidptr_t address, /*in*/ uint64_t duration, /*in*/ uint32_t stackSize, /* out */ IEcoTask1** ppITask) {
    /*CEcoTaskScheduler1Lab_C761620F* pCMe = (CEcoTaskScheduler1Lab_C761620F*)me;*/
    int32_t indx = 0;
    int32_t reg = 30;
    ...
    while (1) {
		...
		for (i = 0; i < MAX_STATIC_TASK_COUNT; ++i) {
			if (pCMe->m_pTaskList[i].pfunc != 0 &&
				(pCMe->m_pTaskList[i].duration < min_duration || min_duration == 0)) {
				g_indx = i;
				min_duration = pCMe->m_pTaskList[i].duration;
			}
		}
		pCMe->m_pTaskList[g_indx].pfunc(min_duration);
